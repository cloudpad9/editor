...
function git_diff($builder) {
    ...
    if (empty($info) || empty($info['ok'])) {
        $msg = $info['message'] ?? 'Not a git repository or cannot determine repository root.';
    ...
    $out = '';
    $ok  = $builder->execGitCommand($toplevel, '--no-pager diff --no-color -- ' . escapeshellarg($relpath), $out);
