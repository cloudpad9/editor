...
<div id="tabs-snr-inner"<?php echo session_get('snr-batch-mode') ? ' class="batch-mode"' : ''; ?>>
    <div class="editor-file-bar commandbar">
        <form data-success="onSearchDone" data-container="#snr-response" data-verbose="1" action="index.php" method="POST" enctype="multipart/form-data" style="float:left;">
            ...
            <div class="moz-select-wrapper">
                <select name="repository" class="repositories">
                    ...
                    <?php foreach ($repositories as $code => $settings) : ?>
                        <option value="<?php echo $code; ?>" <?php echo session_get('snr-repository') == $code? 'selected' : ''; ?>><?php echo $settings['name']; ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <span id="snr_search_wrapper">
                <input id="snr_search" type="text" class="filename" name="search" value="<?php echo htmlspecialchars((string)session_get('search', ''), ENT_QUOTES, 'UTF-8'); ?>" placeholder="Enter a search string"/>
                <textarea id="snr_batch_snr" name="batch-snr" placeholder="or multiple search => replace lines here..."><?php echo htmlspecialchars((string)session_get('batch-snr', '')); ?></textarea>
            </span>
            ...
            <input type="text" name="max-files-returned" value="<?php echo session_get('max-files-returned', '10'); ?>" size="2"/>
            <input type="text" class="filename" name="file-mask" value="<?php echo session_get('file-mask', '*.*'); ?>" placeholder="*.php;*.tpl;-foo;-bar"/>
            ...
            <input id="snr_replace" type="text" class="filename" name="replace" value="<?php echo htmlspecialchars((string)session_get('replace', ''), ENT_QUOTES, 'UTF-8'); ?>" placeholder="Enter a replacement string"/>
